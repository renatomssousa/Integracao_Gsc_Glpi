...
def atualizar_status_ticket(ticket_id: int, status_id: int) -> None:
    ...
    if r.status_code not in (200, 201):
        # nao quebra o loop por causa disso
        return
...
def adicionar_followup_com_anexo(
    ...
    files = {
        "_filename[]": (filename, BytesIO(file_bytes)),
    }
...
def listar_documentos_ticket(ticket_id: int) -> List[Dict[str, Any]]:
    """
    Tenta listar documentos ligados ao Ticket.
...
def _get_document_meta(doc_id: int) -> Optional[Dict[str, Any]]:
    r = requests.get(
...
def baixar_documento_base64(doc_id: int) -> str:
    ...
    # tentativa 2 (algumas instalações usam /download)
    r2 = requests.get(
