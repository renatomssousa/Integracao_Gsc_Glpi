...
def _find_text_any_namespace(node, tag_name: str) -> str:
    """
    Procura um texto dentro de qualquer namespace:
    Ex: <ns0:descricao>...</ns0:descricao>
    """
    if node is None:
        return ""
...
def extrair_reiteracoes(xml: str):
    ...
    for item in root.iter():
        ...
        # campos que às vezes existem e ajudam a identificar a reiteracao
        ...
        # tenta pegar anexos se vierem na reiteracao
        anexos = []
        anexos_node = None
        for el in item.iter():
        ...
        # ID unico/fingerprint da reiteracao (evita bloquear por REQ/WO apenas)
