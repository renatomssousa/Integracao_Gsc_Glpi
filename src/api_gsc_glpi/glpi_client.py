...
def listar_documentos_ticket(ticket_id: int) -> List[Dict[str, Any]]:
    r = _request("GET", f"/Ticket/{ticket_id}/Document_Item")
    r.raise_for_status()
    data = r.json()
    return data if isinstance(data, list) else data.get("data", [])


def buscar_metadados_documento(doc_id: int) -> Dict[str, Any]:
    r = _request("GET", f"/Document/{doc_id}")
    r.raise_for_status()
    data = r.json()
...
def baixar_documento_base64(doc_id: int) -> str:
    ...
    if r.status_code >= 400:
        raise Exception(f"Erro ao baixar documento do GLPI: status={r.status_code} body={r.text}")
