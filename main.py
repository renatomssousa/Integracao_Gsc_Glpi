...
from config import POLL_INTERVAL_SECONDS, GLPI_CATEGORIA_ID
from utils import limpar_html_glpi

ENABLE_GLPI_ANEXOS_PARA_CAIXA = os.getenv("ENABLE_GLPI_ANEXOS_PARA_CAIXA", "0").strip() in ("1", "true", "True")

# marcador anti-loop (CAIXA -> GLPI)
ORIGEM_CAIXA_TAG = "[ORIGEM_CAIXA]"
    ...
    while True:
        try:
            ...
            for ch in aberturas:
                ...
                for ax in ch.get("anexos", []):
                    ...
                    glpi_client.adicionar_followup_com_anexo(
                        ticket_id=ticket_id,
                        texto=f"{ORIGEM_CAIXA_TAG}\nArquivo anexado automaticamente (origem: CAIXA)",
                        filename=nome,
                        base64_file=b64,
                    )

            # 2) REITERACOES (CAIXA -> GLPI)
            ...
            for r in reiteracoes:
                ...
                # se estava solucionado/fechado e chegou nova intera��o da CAIXA -> volta para Em atendimento (2)
                try:
                    st = glpi_client.buscar_status_ticket(ticket_id)
                    if st in ("Solucionado", "Fechado"):
                        glpi_client.atualizar_status_ticket(ticket_id, 2)
                except Exception:
                    pass

                # followup publico com a nota da CAIXA (anti-loop)
                ...
                if final:
                    ...
                    glpi_client.adicionar_followup_publico(
                        ticket_id,
                        f"{ORIGEM_CAIXA_TAG}\nIntegracao: REQ/WO bloqueada porque a CAIXA informou {final}.",
                    )
            ...
            tickets = glpi_client.buscar_tickets_caixa_por_categoria(GLPI_CATEGORIA_ID)
            ...
            for t in tickets:
                ...
                for f in followups:
                    ...
                    texto_raw = (f.get("content") or "").strip()
                    if not texto_raw:
                        glpi_updates.marcar_followup_enviado(ticket_id, fid)
                        continue

                    # ? anti-loop: se � origem CAIXA, n�o envia pra CAIXA
                    if ORIGEM_CAIXA_TAG in texto_raw:
                        glpi_updates.marcar_followup_enviado(ticket_id, fid)
                        continue

                    # ? remove HTML do GLPI antes de mandar
                    texto = limpar_html_glpi(texto_raw)
                    if not texto:
                        glpi_updates.marcar_followup_enviado(ticket_id, fid)
                        continue
                    ...
                    try:
                        ...
                        glpi_client.adicionar_followup_publico(
                            ticket_id,
                            f"{ORIGEM_CAIXA_TAG}\nIntegracao: CAIXA retornou erro FINAL. REQ/WO bloqueada.",
                        )
                        break
